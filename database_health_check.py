...
def run_diagnostics():
    ...
    try:
        ...
        print(f"✅ User Table: Accessible ({user_count['count']} users found)")
        
        # 2. Check Train data
        train_count = execute_query("SELECT COUNT(*) as count FROM trains", fetchone=True)
        print(f"✅ Train Table: Accessible ({train_count['count']} trains found)")
        
        # 3. Check Booking data
        booking_count = execute_query("SELECT COUNT(*) as count FROM bookings", fetchone=True)
        print(f"✅ Bookings Table: Accessible ({booking_count['count']} records found)")
        
        print("-" * 40)
        print("🎉 All systems green. Database is healthy.")
